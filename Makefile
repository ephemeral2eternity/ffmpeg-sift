...
# Add by WCC, cbcd Group ------ Deal with opencv lib
OPENCVLIBS = -L/usr/lib -lcxcore -lcv -lhighgui -lcvaux -lml
OPENCVLDFLAGS = -Wl,-rpath,/usr/local/lib
OPENCVINCLUDE = -I/usr/include/opencv/
LDFLAGS += $(OPENCVLDFLAGS)

# Add by WCC, cbcd Group ------ Deal with mysql lib
MYSQLLIBS = -L/usr/include/mysql -L/usr/lib/mysql -lmysqlclient -lz
...
FFLIBS := avdevice avformat avcodec avutil cbcd

include common.mak

FF_LDFLAGS   := $(FFLDFLAGS)
FF_LDFLAGS   += $(OPENCVLIBS)
FF_LDFLAGS   += $(MYSQLLIBS)
...
uninstall-progs:
	rm -f -r $(addprefix "$(BINDIR)/", $(ALLPROGS))

uninstall-man:
	rm -f -r $(addprefix "$(MANDIR)/man1/",$(ALLMANPAGES))

uninstall-vhook:
	rm -f -r $(addprefix "$(SHLIBDIR)/",$(ALLHOOKS_SRCS:.c=$(SLIBSUF)))
