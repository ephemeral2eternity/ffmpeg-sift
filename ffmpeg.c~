...
// Add by WCC, CBCD group, 2009-0409
    ...
    handle_eof:
        ...
        if (ist->decoding_needed) {
            ...
            case CODEC_TYPE_VIDEO:
                    ...
                    // if(ist->st->codec->frame_number % 10 == 0)		// CBCD group, WCC-2009-0513
		    if (picture.pict_type == FF_I_TYPE)				// CBCD group, revised by WCC-090603
		    {
                      sift_main(picture.data[0] , ist->st->codec->width , ist->st->codec->height , picture.linesize[0], ist->st->codec->frame_number, shortFileName);
		      if (sampleNum < 11)
		      {
			sampleNum ++;
		      }
                ...
                if (ost->source_index == ist_index) {
                    ...
                    if (ost->encoding_needed) {
                        ...
                        case CODEC_TYPE_AUDIO:
                            ...
                            break;
                        case CODEC_TYPE_VIDEO:
                            // do_video_out(os, ost, ist, &picture, &frame_size);	// Comment by CBCD group, wcc-090609
...
/************************ MySQL Connection, WCC, cbcd group, 2009-0619 *******************************/
	...
	int tmp_i = 0, tmp_j = 0;
	char mysqlOrder[200];
	strcpy( orgFileName, input_files[0]->filename);
	while( orgFileName[tmp_i] != '.' )
	{
	tmp_i ++;
	}
	while( orgFileName[tmp_i-tmp_j] != '/')
	{
	tmp_j ++;
	}
	strncpy(shortFileName, orgFileName + tmp_i - tmp_j + 1, tmp_j - 1);

	// Create des table
	snprintf(mysqlOrder, sizeof(mysqlOrder), "CREATE TABLE sift_des_%s LIKE sift_des", shortFileName);
	if (mysql_query (cnx_init, mysqlOrder) != 0 )
	{
		printf("The video %s has been extracted because the des table already existes!!!\n", shortFileName);
		return -1;
	}
	
	// Create frm table
	snprintf(mysqlOrder, sizeof(mysqlOrder), "CREATE TABLE sift_frm_%s (x DOUBLE, y DOUBLE, scl DOUBLE, ori DOUBLE)", shortFileName);
	if (mysql_query (cnx_init, mysqlOrder) != 0 )
	{
		printf("The video %s has been extracted because the frm table already existes!!!\n", shortFileName);
		return -1;
	}

	// Create ind table
	snprintf(mysqlOrder, sizeof(mysqlOrder), "CREATE TABLE sift_ind_%s (frmNUM BIGINT UNSIGNED, startLN BIGINT UNSIGNED, endLN BIGINT UNSIGNED, siftNUM INT)", shortFileName);
	if (mysql_query (cnx_init, mysqlOrder) != 0 )
	{
		printf("The video %s has been extracted because the ind table already existes!!!\n", shortFileName);
		return -1;
	}
	printf("Processing video %s.mpg now!!!!!!!!!\n", shortFileName);
	sampleNum = 0;
	// Add End, WCC, CBCD group, 2009-0409    
/************************** MySQL Connection End, WCC, cbcd group, 2009-0619 *************************/
...
/********************** MySQL Connection Close, WCC, cbcd group, 2009-0619 ***************************/
	mysql_close(cnx_init);
	// printf("Terminating connecting to cbcd_feat_sift!\n");
/******************* MySQL Connection Close End, WCC, cbcd group, 2009-0619 **************************/
